...
def main():
    ...
    parser.add_argument('dataset', choices=['oxiod', 'euroc', 'synthetic', 'synthetic9d'], help='Training dataset name (\'oxiod\' or \'euroc\')')
    ...
    if args.dataset == 'synthetic9d':
        cur_gyro_data, cur_acc_data, cur_mag_data, cur_pos_data, cur_ori_data = load_synthetic_9d_dataset("dataset")
        ...
        if args.dataset == 'synthetic':
            cur_gyro_data, cur_acc_data, cur_pos_data, cur_ori_data = load_synthetic_dataset("dataset")
    ...
    if args.dataset == 'synthetic9d':
        ...
        train_model = create_train_resnet_model_9d_quat(pred_model, window_size)
    else:
    ...
    if args.dataset == 'synthetic9d':
        history = train_model.fit([x_gyro, x_acc, x_mag, y_delta_p, y_delta_q], epochs=50, batch_size=32, verbose=1, callbacks=[model_checkpoint, tensorboard], validation_split=0.1)
        pred_model = create_resnet_pred_model_9d_quat(window_size)
    else:
