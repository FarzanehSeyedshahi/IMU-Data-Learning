...
def main():
    ...
    parser.add_argument('dataset', choices=['oxiod', 'euroc', 'IMUData'], help='Training dataset name (\'oxiod\' or \'euroc\')')
    ...
    if args.dataset == 'IMUData':
        dataset_path = '/home/work/fshahi/paola/'
        file_names_sentences = "221006_IMU_SL_*.csv"
        # file_names_words = "SignLanguage_221006_Paola_*.csv"
        file_names_words = "SignLanguage_221006*.csv"
        file_names_words_sentences = "*.csv"

        x_accR, x_accL, x_gyroR, x_gyroL, x_magR, x_magL, y_poseR, y_poseL, y_orientationRmR, y_orientationRmL, y_orientationQR, y_orientationQL = load_dataset_IMUData(dataset_path, file_names_words_sentences)
        
        #Right hand
        # [x_gyro, x_acc, x_mag], [y_delta_p, y_delta_rm], init_p, init_q = load_dataset_9d_rm(x_accR, x_gyroR, x_magR, y_poseR, y_orientationRmR, window_size, stride)
        #Left hand
        [x_gyro, x_acc, x_mag], [y_delta_p, y_delta_rm], init_p, init_q = load_dataset_9d_rm(x_accL, x_gyroL, x_magL, y_poseL, y_orientationRmL, window_size, stride)

        y_delta_q = y_delta_rm
...
#     train_model = create_train_resnet_or_without_model_9d_quat(pred_model, window_size= window_size_transformer)
    ...
    model_checkpoint = ModelCheckpoint('models/{}_checkpoint.hdf5'.format(args.output), monitor='val_loss', save_best_only=True, verbose=1)
    tensorboard = TensorBoard(log_dir="logs/{}_{}".format(args.output, time()))
    early_stopping = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=20)

    # 9D input
#     history = train_model.fit([x_gyro, y_delta_p, y_delta_q], epochs=epochs, batch_size=32, verbose=1, callbacks=[model_checkpoint, tensorboard, early_stopping], validation_split=0.1)
    ...
    train_model = load_model('models/{}_checkpoint.hdf5'.format(args.output), custom_objects={'CustomMultiLossLayer':CustomMultiLossLayer}, compile=False)
